...
def _resolve_iri_slug(train_no: str, session: curl_requests.Session) -> str:
    ...
    # STEP 0: Solve IRI challenge FIRST to get verified session cookies for cloud IPs
    try:
        r_root = session.get("https://srv2.indiarailinfo.com/", headers=headers, timeout=8)
        if "iri-xsig" in r_root.text:
            soup_root = BeautifulSoup(r_root.text, "html.parser")
            xsig_div = soup_root.find("div", id="iri-xsig")
            if xsig_div:
                xsig = xsig_div.get("data-sig")
                x_val = xsig.split("|")[-1] if "|" in xsig else "24"
                verify_url = f"https://srv2.indiarailinfo.com/verify-browser?t=0:5:2:1:8:1:1:0:{x_val}:{xsig}:0"
                session.get(verify_url, headers=headers, timeout=8)
                time.sleep(0.5)
    except Exception as e:
        print(f"  [WARN] Challenge pre-solve failed: {e}")

    # Strategy 1: IndiaRailInfo Official Direct Internal Autocomplete API
    try:
        url_internal = f"https://srv2.indiarailinfo.com/shtml/list.shtml?LappGetTrainList/{train_no}/0/0/0"
    ...
    # Strategy 2: DuckDuckGo HTML Search Indexing (Cloud Safe Fallback)
    try:
    ...
    # Strategy 3: Yahoo Search Fallback
    try:
...
def fetch_train_railmitra(train_no: str) -> dict | None:
    """
    Fetch train schedule from railmitra.com - Cloud IP safe (no bot protection).
    Returns same dict structure as fetch_train_indiarailinfo.
    """
    import re as _re
    ...
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36",
        "Referer": "https://railmitra.com/"
    }

    try:
        url = f"https://railmitra.com/train-schedule/{train_no}"
        r = session.get(url, headers=headers, timeout=12)
        if r.status_code != 200 or len(r.text) < 3000:
            print(f"  [RAILMITRA] HTTP {r.status_code} or too small for {train_no}")
            return None
        ...
        # Train name from H1
        h1 = soup.find("h1")
        train_name_raw = h1.get_text().strip() if h1 else f"Train {train_no}"
        # Clean name: "22363-AMRIT BHARAT EXP Train Schedule" -> "Amrit Bharat Express"
        train_name = _re.sub(r'^\d+[-\s]*', '', train_name_raw)
        train_name = _re.sub(r'\s*Train Schedule\s*$', '', train_name, flags=_re.IGNORECASE).strip().title()

        # Find the main schedule table (first table with STATION NAME header)
        schedule_table = None
        for table in soup.find_all("table"):
            headers_row = table.find("tr")
            if headers_row:
                header_texts = [th.get_text().strip().upper() for th in headers_row.find_all(["th", "td"])]
                if "STATION NAME" in header_texts and "CODE" in header_texts:
                    schedule_table = table
                    break

        if not schedule_table:
            print(f"  [RAILMITRA] No schedule table found for {train_no}")
            return None
        ...
        for row in rows:
            ...
            if len(tds) < 5:
                continue
            stn_name = tds[0].get_text().strip().title()
            stn_code = tds[1].get_text().strip().upper()
            arr = tds[2].get_text().strip()
            halt = tds[3].get_text().strip()
            dep = tds[4].get_text().strip()
            day = tds[5].get_text().strip() if len(tds) > 5 else "1"
            dist = tds[6].get_text().strip() if len(tds) > 6 else "0"

            # Normalize times: "16.10" -> "1610 hrs"
            def norm_time(t):
                t = t.strip()
                if t.lower() in ("first", "last", "-", ""):
                    return t.title() if t else "-"
                t_clean = t.replace(".", "").replace(":", "")
                if t_clean.isdigit() and len(t_clean) in (3, 4):
                    return t_clean.zfill(4) + " hrs"
                return t

            arr_norm = norm_time(arr)
            dep_norm = norm_time(dep)
        ...
        # Extract origin/dest from first/last row
        origin_code = stations[0]["stn_code"]
        dest_code = stations[-1]["stn_code"]
        dep_time = stations[0]["dep"]
        arr_time = stations[-1]["arr"]

        print(f"  [RAILMITRA] Train {train_no} | {train_name} | {origin_code}->{dest_code} | Dep: {dep_time}")
        ...
        print(f"  [RAILMITRA] Error for {train_no}: {e}")
        return None
...
def fetch_train_indiarailinfo(train_no: str) -> Optional[dict]:
    ...
    try:
        ...
        if not html or len(html) < 5000:
            ...
            # Fallback to railmitra.com (cloud-safe)
            print(f"  [RAILMITRA FALLBACK] Trying railmitra.com for {train_no}...")
            result = fetch_train_railmitra(train_no)
            if result:
                return result
