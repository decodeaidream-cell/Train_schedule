...
def _solve_iri_challenge_and_fetch(url: str, session: curl_requests.Session) -> str:
    url = url.replace("https://indiarailinfo.com", "https://srv2.indiarailinfo.com").replace("https://m.indiarailinfo.com", "https://srv2.indiarailinfo.com")
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36",
        "Referer": "https://srv2.indiarailinfo.com/"
    }
    ...
    try:
        ...
        if "iri-xsig" in r.text:
            ...
            if xsig_div:
                ...
                verify_url = f"https://srv2.indiarailinfo.com/verify-browser?t=0:5:2:1:8:1:1:0:{x_val}:{xsig}:0"
...
def _resolve_iri_slug(train_no: str, session: curl_requests.Session) -> str:
    ...
    # Strategy 1: DuckDuckGo HTML Search Indexing (Cloud Datacenter Safe)
    try:
        ...
        print(f"  [WARN] DDG search slug strategy failed for {train_no}: {e}")

    # Strategy 2: Yahoo Search Indexing
    try:
        y_url = f"https://search.yahoo.com/search?p=site:indiarailinfo.com+train+{train_no}"
        r_y = session.get(y_url, headers=headers, timeout=6)
        ...
        print(f"  [WARN] Yahoo search slug strategy failed for {train_no}: {e}")

    # Strategy 3: Bing Search Indexing
    try:
        url_b = f"https://www.bing.com/search?q=site:indiarailinfo.com+train+{train_no}"
        r_b = session.get(url_b, headers=headers, timeout=6)
        if r_b.status_code == 200:
            soup_b = BeautifulSoup(r_b.text, "html.parser")
            for a in soup_b.find_all("a"):
                href = a.get("href", "")
                if "indiarailinfo.com/train/" in href:
                    tid = _extract_id_from_url(href, train_no)
                    if tid:
                        return tid
    except Exception as e:
        print(f"  [WARN] Bing search slug strategy failed for {train_no}: {e}")

    # Strategy 4: srv2.indiarailinfo.com Search with JS Verification Solver
    try:
        search_url = f"https://srv2.indiarailinfo.com/trains?q={train_no}"
        html_s = _solve_iri_challenge_and_fetch(search_url, session)
        ...
        print(f"  [WARN] d.indiarailinfo search slug strategy failed for {train_no}: {e}")
...
def fetch_train_indiarailinfo(train_no: str) -> Optional[dict]:
    ...
    Scrape train schedule directly from IndiaRailInfo with dynamic JS verification solver.
    ...
    train_id = _resolve_iri_slug(train_no, session)

    iri_url = f"https://srv2.indiarailinfo.com/train/{train_id}"
